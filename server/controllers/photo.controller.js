...
module.exports.findAllPhotos = (req, res) => {
    Photo.find()
        .then(allPhotos => res.json(allPhotos))
        .catch(err => res.json({ message: "Something went wrong", error: err }));
};

module.exports.findAllPhotosAnon = (req, res) => {
    Photo.find({ show_anonymous: true })
        .then(allPhotos => res.json(allPhotos))
        .catch(err => res.json({ message: "Something went wrong", error: err }));
};
...
module.exports.findAllPhotosAnonByGalleryID = (req, res) => {
    Photo.find({ _id: req.params.id, show_anonymous: true })
        .then(photos => res.json(photos))
        .catch(err => res.json({ message: "Something went wrong", error: err }));
};
